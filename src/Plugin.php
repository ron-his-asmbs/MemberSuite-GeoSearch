...
class Plugin
{
    ...
    private function executeSearchDirectoryIndividuals(): array
    {
        ...
        $this->log('ms_sync: MS_SURGEON = ' . $ms_surgeon);
        $this->log('ms_sync: MS_INTEGRATED_HEALTH = ' . $ms_integrated_health);
        $this->log('ms_sync: token = ' . (empty($token) ? 'EMPTY' : 'ok'));
        ...
        if (is_wp_error($response)) {
            $this->log('ms_sync: API error: ' . $response->get_error_message());
            return [];
        }
        ...
        $this->log('ms_sync: status code = ' . $status);
        $this->log('ms_sync: response body = ' . substr($body, 0, 500));
    ...
    public function member_search_shortcode(): string
    {
        ...
        <div id="ms-member-search">
            <div id="ms-search-form" style="margin-bottom: 20px;">
                ...
                <button id="ms-search-btn" style="padding: 8px 16px; font-size: 16px; margin-left: 10px; cursor: pointer;">
                    Search
                </button>
            </div>
        ...
        <script src="https://maps.googleapis.com/maps/api/js?key=<?php echo esc_attr($api_key); ?>&callback=initMap" async
            defer>
        </script>
